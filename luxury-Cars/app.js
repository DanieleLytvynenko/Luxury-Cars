  ...
  if (!cars.length) {
    carsGrid.innerHTML = `
      <div style="grid-column: 1/-1; text-align:center; padding: 60px 20px; color: var(--gray)">
        <p style="font-size:1rem">Автомобили не найдены</p>
      </div>`;
    return;
  }
    ...
    card.innerHTML = `
      ...
      <div class="car-card__body">
        ...
        <div class="car-card__cta">
          <button class="car-card__book" data-car="${escHtml(car.title || '')}" aria-label="Забронировать ${escHtml(car.title || 'автомобиль')}">
            Забронировать
          </button>
        </div>
      </div>`;
